...
async function publicKey() {
    try {
        ...
        var pubHex = await secUtl.publicKeyHex(keyPairHex.privateKeyHex)
        var pubBytes = await secUtl.publicKeyBytes(secUtl.createKeyPairBytes.privateKeyBytes)
        ...
        if(isMatchHex && isMatchBytes) {
            ...
            before = stamp()
            while(c--) {
                pubHex = await secUtl.publicKeyHex(keyPairHex.privateKeyHex)
            }
            after = stamp()
            hexMS = after - before
            ...
            before = stamp()
            while(c--) {
                pubBytes = await secUtl.publicKeyBytes(keyPairBytes.privateKeyBytes)
            }
            after = stamp()
            bytesMS = after - before            

            results.testShas = { success, bytesMS, hexMS }
